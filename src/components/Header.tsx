...
const Header: React.FC<HeaderProps> = ({ toggleSidebar }) => {
  ...
  return (
    <motion.header
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      className={`w-full transition-all duration-300 ${scrolled ? 'bg-white shadow-lg backdrop-blur-sm' : 'bg-white/95'
        }`}
    >
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {navItems.map(({ path, label, icon: Icon }) => (
              <Link
                key={path}
                to={path}
                className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg transition-all duration-200 ${location.pathname === path
